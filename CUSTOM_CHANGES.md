...
- 实测确认：关掉无障碍服务，微信通话录音完全不受影响——证实这个服务对现有功能确实没有
  任何作用，是纯历史遗留。`WeChatCallAccessibilityService.kt` 和它的配置文件
  `wechat_accessibility_service_config.xml` 已删除，manifest 里对应的 `<service>` 声明
  也一并删掉。
- 现在微信通话录音功能**只需要"通知使用权"**这一个特殊权限，不再需要无障碍。

## 17. 新增诊断专用的 `WeChatCallActivityProbe`（临时性质，用完可删）

- 背景：已经确认延迟的根本原因是微信自己有时候接通变慢（隔一段时间后第一通电话明显比
  连续拨打的电话慢），不是我们的检测代码或者系统进程调度的问题。实测确认微信语音/视频
  通话振铃、通话中用的都是同一个 Activity——`com.tencent.mm.plugin.voip.ui.VideoActivity`。
- 新增 `WeChatCallActivityProbe`（一个全新的、专门为这一次诊断目的写的无障碍服务，
  跟已删除的旧无障碍服务没有代码上的关系），只做一件事：记录微信这个 Activity 每次窗口
  状态变化的类名和精确时间戳，不读取任何窗口内容（不需要，也早就确认读不到）。
- 目的：如果这个 Activity 出现的时间点，比"语音通话中"这条通知明显更早（哪怕只早几秒），
  就值得把检测触发点从"等通知"换成"等这个 Activity 出现"——因为多录几秒微信还在连接时的
  安静片段完全无害，但少录几秒真实内容是实打实的损失。如果这个 Activity 出现的时间点跟
  通知差不多甚至更晚，说明微信自己的"连接慢"这个瓶颈，在这个 Activity 真正显示出来之前
  就已经发生了，换了也没用。
- 这是**纯诊断代码，不影响任何现有功能**，也不会被现有代码引用或依赖。测试完得出结论后，
  不管结论是"值得换"还是"没用"，这个探测器本身都应该删掉或者改造成正式功能，不会长期
  保留现在这个诊断形态。

## 18. 修复文件名 "xxx.wav.wav" 重复后缀的 bug + 改成可读时间格式

- **Bug 原因**：`OutputDirUtils.moveToOutputDir()`（真实电话录音也在用的同一个工具函数）
  设计上是"给它不带后缀的文件名，它自己根据传入的文件类型（MIME type）去加后缀"。之前
  微信录音这边传的文件名已经自带了 `.wav`，导致它又在后面加了一次，变成
  `xxx.wav.wav`。这是调用方式的问题，不是这个工具函数本身有毛病。
- **顺带把命名格式也改了**：以前是纯数字时间戳（`wechat_1789136260044.wav`），现在改成
  `WeChat_2026-09-12_14-30-05.wav` 这种人类可读的年-月-日\_时-分-秒格式，一眼就能看出
  是什么时候录的。

## 19. 设置界面文案调整

- 新增的"微信通话自动录音"开关，标题、简介、点击 ⓘ 弹出的详细说明，全部补上了简体中文
  翻译（之前只写了英文原文，中文设备会显示英文兜底，因为没配对应的中文翻译资源）。样式
  跟"通话悬浮录音球"那一项保持一致（标题旁边一个 ⓘ 图标，点击弹对话框）。
- "保存格式"这一项标题改成"**保存格式（不含微信录音）**"，避免用户误以为这个格式选项
  也能控制微信录音的输出格式——微信录音现在固定是 WAV，不受这个设置影响。

## 20. 正式启用 Activity 探测信号：预热 + 通知确认 + 超时丢弃

- **实测结论**：`WeChatCallActivityProbe` 在真实"慢热"通话上，比通知早了近 17 秒触发；
  同时经过多轮日常使用测试（聊天、朋友圈、摄像头、视频号、小程序），一次误触发都没有。
  信号本身足够干净，值得正式用起来。
- 但考虑到没法 100% 保证以后绝不会有意想不到的场景触发它，没有直接把它当成唯一依据，
  而是设计成"预热 → 等通知确认 → 超时兜底丢弃"这一套状态机，全部实现在
  `WeChatCallNotificationListenerService` 里：
  1. **探测器触发**（`onProbeEvent`）：如果当前没有正在跟踪的通话，立刻弹出悬浮球；
     如果开着"自动录音"，同时立刻开始一段**待确认**的录音，并启动 30 秒倒计时
     （`PENDING_TIMEOUT_MS`，比实测过的最长 17 秒延迟留了近一倍余量）。
  2. **通知confirmed**（`onNotificationPosted`）：如果这之前已经因为探测器信号在录音/
     弹球了，就只是取消倒计时、把这次标记为"已确认"，不会重复弹球或重复开始一段新录音；
     如果探测器没触发过（比如探测器的无障碍权限没开），就走原来的老逻辑，直接根据通知
     反应。
  3. **倒计时到点还没等到通知**（`onPendingTimeout`）：
     - 如果是自动模式下、还没被用户碰过的"待确认"录音——判定为误报，调用
       `WeChatCallCaptureService.stopAndDiscard()` 停止并**直接删除**这段录音，收起悬浮球
     - 如果用户在倒计时期间**已经手动点过悬浮球**（不管是开始还是停止）——完全不碰，
       尊重用户的手动决定，不会因为倒计时到了就去打扰这段录音
     - 如果是手动模式、悬浮球弹出来后用户全程没点过——收起悬浮球，当作误报清理掉
- `WeChatCallCaptureService` 新增 `stopAndDiscard()` 静态方法：跟正常的 `stopService()`
  走的是同一个服务实例，区别在于停止前会标记一个 `discardOnStop` 标志，`onDestroy()` 里
  看到这个标志就直接删掉裸 PCM 临时文件，不走"封装 WAV → 移动到输出目录"这一套，不会在
  `WeChat` 文件夹里留下痕迹。
- 手动点击悬浮球（`toggleRecording`）无论发生在"待确认"阶段还是"已确认"阶段，都会立刻
  把这次录音标记为"不可丢弃"——手动操作的优先级永远高于自动判断的安全网。

## 21. 撤回第 20 节的"提前触发"机制

- **实测证伪**：连续多次真实通话测试（跨设备），"提前弹出悬浮球"这个效果一次都没有复现
  ——之前那次"提前 17 秒"的记录，现在回头看更像是巧合（当时可能是别的操作恰好也触发了
  同一种窗口事件），不是通话本身的可靠预兆。
- **新发现的副作用**：微信冷启动、开屏动画的时候，同样会触发这个"窗口状态变化"事件，
  导致悬浮球会莫名其妙自己弹出来又自己收起——这证明这个事件本身就不是"通话专属"的信号，
  只是微信内部换页面时经常触发的一种通用事件，日常使用中随时可能撞见。
- 结论：这个功能"承诺的好处没兑现，还引入了实打实的误判"，果断撤回。删除了
  `WeChatCallActivityProbe`、对应的无障碍配置文件、`WeChatCallCaptureService` 里的
  `stopAndDiscard()`，`WeChatCallNotificationListenerService` 退回到纯粹靠通知检测的
  版本（等价于 v3.103 的行为），"微信通话自动录音"这个开关本身保留，不受这次撤回影响。
- 关于国产手机怎么做到"通知出现前就有悬浮球"的——分析下来（没有实锤，是合理推测）：那类
  功能大概率是设备厂商自己预装、用厂商平台签名的系统应用，能拿到的权限和数据访问能力，
  跟我们这种第三方 App（哪怕借助 Magisk 提到特权级别）不是一个量级，可能不是"技术窍门"
  的问题，是"身份"的问题，我们大概率复现不了。

## 22. 新增：可自定义的微信通话检测关键词

- 背景：第 21 节撤回后，检测机制回到"死盯着微信通知文字里有没有出现固定的几个字"这种
  笨办法，天生脆弱——哪天微信改了措辞，检测会毫无征兆地失效。这次加一个自助补救的入口：
  把原来写死在代码里的关键词列表，改成用户可以自己在设置里编辑的一项。
- 设置页"微信通话自动录音"下面新增"**微信通话关键词**"这一项：
  - 副标题直接显示当前生效的关键词（默认"语音通话中,视频通话中"）
  - 点这一行本身，弹出可编辑的输入框，**预填当前值**（不是空白），多个关键词用英文逗号
    分隔，只要通知文字包含其中任意一个就判定为通话；对话框里还有"恢复默认"按钮，一键填回
    内置的两个默认关键词
  - 标题旁边的 ⓘ 图标，点开是纯说明性的对话框，解释这个机制为什么脆弱、遇到失效了该怎么
    自己动手修，跟"通话悬浮录音球"/"微信通话自动录音"两个 ⓘ 是同一个交互逻辑
- `Preferences.wechatCallKeywords`（原始字符串）+ `Preferences.wechatCallKeywordsList`
  （解析成列表，自动去空白、去空项）：如果解析完是空列表（比如用户不小心把输入框清空了），
  自动退回内置默认的两个关键词，不会因为设置被清空就导致整个检测功能悄无声息地彻底失效。
- `WeChatCallNotificationListenerService` 里原来写死的 `CALL_TEXT_KEYWORDS` 常量已删除，
  改成每次都读 `Preferences(this).wechatCallKeywordsList`。

## 23. 文案微调，避免和真实电话的设置混淆

- "自动录音规则""最短录音时长"标题都加上了"（不含微信录音）"，跟"保存格式"那次的处理
  方式一样——这几个设置只对真实电话生效，微信录音不受影响。
- "微信通话关键词"改名为"**微信通话关键词（可编辑）**"，避免看起来像一个纯展示、不能
  点的信息条目。

## 24. 拆分悬浮球开关：系统电话 / 微信电话各自独立

- **顺带发现一个之前的疏漏**：真实电话弹悬浮球之前会检查"通话录音悬浮球"这个开关，但
  微信这边弹悬浮球之前**从来没检查过这个开关**，是无条件弹的。这次一并修正。
- "通话悬浮录音球"改名为"**系统通话录音球**"，只管真实电话，行为不变。
- 新增"**微信电话录音球**"（`Preferences.wechatFloatingButtonEnabled`，默认开启，跟
  加这个开关之前"无条件弹出"的老行为保持一致），放在"微信通话自动录音"下面，单独控制
  微信通话时弹不弹悬浮球——关掉它不影响"微信通话自动录音"或者下面第 25 节的磁贴照常
  录音，只是不会再弹出悬浮球做视觉提示。两个开关互不影响，各自都带 ⓘ 说明。

## 25. 新增：微信录音磁贴（通知栏下拉面板），从根源上绕开"缺前几秒"的问题

- 背景：所有"自动检测通话再触发录音"的方案，卡的都是同一个死结——软件没法在通话真正
  发生之前，凭空知道它要发生。这次换一个思路：不再让软件去猜，而是让用户自己在**打电话
  之前**主动点一下，把决定权交还给用户，从根源上绕开检测延迟的问题。
- 项目里本来就有一个磁贴功能（`RecorderTileService`，管真实电话自动录音功能的整体开关），
  这次照着它的架构新增了一个独立的磁贴——`WeChatRecorderTileService`：
  - 点一下**直接开始/停止**微信录音，跟任何通话检测完全无关，可以在打电话之前提前点
  - 磁贴自己的图标状态和文字会实时切换（"微信录音" / "**微信录音中…**"），方便你确认
    有没有真的在录，不用非得看到悬浮球才知道
  - 如果提前点开了、但这次其实没打成电话——录音会一直进行，不会自动停止（没有通知可以
    触发"挂断"这件事），需要你自己再点一次磁贴（或者等真的检测到电话、悬浮球弹出来后
    用悬浮球）手动停止
  - 如果提前点开录音后，真的检测到微信通话来了——这次不会重复开始录第二段，会自动"认领"
    这段已经在跑的录音，继续正常走"挂断自动停止"这条路；悬浮球（如果开着）弹出来的时候
    也会直接显示"录音中"的状态，跟磁贴保持同步
- `WeChatCallCaptureService` 新增 `isRunning` 静态状态（谁在跑、谁没在跑，只认这一个
  权威来源），磁贴、悬浮球、自动录音三条路径现在都通过这个状态互相协调，不会出现"各管各
  的、重复启动"的情况。
